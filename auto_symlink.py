...
class AutoSync:
    def __init__(
        ...
        last_sync_path="./config/last_sync.yaml",
    ):
    ...
    def sync_new_list(self):
        ...
        if self.last_sync_list:
            try:
                new_sync_list = [
                    ...
                    if item.get("symlink_dir", "") not in self.last_sync_list
                ]
        ...
        if new_sync_list:
            if sync_enabled:
                ...
                total_time = self.auto_symlink(
                    new_sync_list, num_threads, new_sync=True
                )
    ...
    def start_observer(self):
        ...
        if observer_enabled:
            ...
            print_message(
                '未开启监控状态,如需开启,请至config.yaml中将全局设置中的"observer_enabled"设为"true"'
            )
    ...
    def restart_sync(self):
        if not self.yaml_data:
            ...
            return
        sync_enabled = self.yaml_data.get("sync_enabled", False)  # 默认同步关闭
        restart_sync_enabled = self.yaml_data.get(
            "restart_sync_enabled", False
        )  # 默认重启同步关闭
        if not restart_sync_enabled:
            print_message(
                '未开启重启全同步状态,如需开启,请至config.yaml中将全局设置中的"restart_sync_enabled"设为"true"'
            )
            return
        ...
        if self.last_sync_list:
            try:
                existing_sync_list = [
                    item
                    for item in sync_list
                    if item.get("symlink_dir", "") in self.last_sync_list
                ]
            except Exception as e:
                print_message(f'配置文件"./config/last_sync.yaml错误",请检查配置文件后重启容器.')
        else:
        ...
        if existing_sync_list:
            restart_sync_list = [
                item
                for item in existing_sync_list
                if item.get("restart_sync_enabled", False)
            ]
        else:
            restart_sync_list = []
        ...
        if restart_sync_list:
            if sync_enabled:
                ...
                print_message(
                    '未开启全局同步状态,如需开启,请至config.yaml中将全局设置中的"sync_enabled"设为"true"'
                )
        else:
    ...
    def start_backup_scheduled(self):
        ...
        if not backup_scheduled:
            print_message(
                '未开启定时备份,如需开启,请至config.yaml中将全局设置中的"backup_scheduled"设为"true"'
            )
            return
        # 只定时备份已经全同步过的目录
        sync_list = self.yaml_data.get("sync_list", [])
        ...
        if existing_sync_list:
            sync_backup_list = [
                item
                for item in existing_sync_list
                if item.get("backup_scheduled", False)
            ]
        else:
            sync_backup_list = []
        self.set_scheduled(
        ...
        if cron.count(" ") == 4:
            ...
            try:
                self._scheduler.add_job(
                    func=func, args=[sync_list], trigger=CronTrigger.from_crontab(cron)
                )
            ...
            try:
                ...
                for func_name in self.func_list:
                    if func_name == "MetadataCopyer" and metadata_copyer_flag:
                        metadata_copyer = MetadataCopyer(
                            media_dir, symlink_dir, metadata_ext, num_threads
                        )
                    ...
                    if func_name == "SymlinkDirChecker" and symlink_dir_checker_flag:
                        symlink_dir_checker = SymlinkDirChecker(
                            cloud_path,
                            symlink_dir,
                            media_dir,
                            num_threads,
                    ...
                    if func_name == "SymlinkChecker" and symlink_checker_flag:
                        symlink_checker = SymlinkChecker(
                            cloud_path,
                            media_dir,
                            symlink_dir,
                            symlink_mode,
