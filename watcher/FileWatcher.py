...
class FileMonitor:
    ...
    def add_monitor_conf(self,monitor_confs: list):
        ...
        for monitor_conf in monitor_confs:
            ...
            self._symlink_ext[source_dir] = monitor_conf.get("symlink_ext",".mkv;.iso;.ts;.mp4;.avi;.rmvb;.wmv;.m2ts;.mpg;.flv;.rm;.mov")
            self._metadata_ext[source_dir] = monitor_conf.get("metadata_ext",".nfo;.jpg;.png;.svg;.ass;.srt;.sup")
            self._symlink_creator[source_dir] = monitor_conf.get("symlink_creator",True)
            self._metadata_copyer[source_dir] = monitor_conf.get("metadata_copyer", True)
            self._sync_enabled[source_dir] = monitor_conf.get("sync_enabled", True)
            self._observer_enabled[source_dir] = monitor_conf.get("observer_enabled", True)
            self._symlink_mode[source_dir] = monitor_conf.get("symlink_mode", "")
            self._clouddrive2_path[source_dir] = monitor_conf.get("clouddrive2_path", "")
